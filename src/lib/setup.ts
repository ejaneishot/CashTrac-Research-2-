...
export const TRANSACTION_HEADERS = [
  'id', 'accountId', 'date', 'description', 'category', 'amount', 'balance',
  'source', 'sourceLink', 'fingerprint', 'importedAt', 'marked', 'invoiceLink',
  'notes', 'updatedAt', 'syncState', 'deletedAt',
]
...
export async function createLedgerSpreadsheet(accountName: string, ledgersFolderId: string): Promise<string> {
  ...
  await valuesUpdate(id, 'Transactions!A1', [TRANSACTION_HEADERS])
